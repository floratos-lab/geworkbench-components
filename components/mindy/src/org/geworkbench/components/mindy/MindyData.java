 ...
 * @author mhall
 * @author oshteynb
 * @version $Id: MindyData.java,v 1.9 2009-04-27 15:49:02 keshav Exp $
 */
		...
		for (MindyResults.MindyResultForTarget result : results) {
			...
			for (MindyResults.MindyResultForTarget.ModulatorSpecificResult specificResult : result) {
				...
				if (score != 0.0){
		            ...
		            ModulatorInfo modInfo = modulatorInfoMap.get(mod);
		            if (modInfo == null) {
		            	modInfo = new ModulatorInfo(mod);
		            	modulatorInfoMap.put(mod, modInfo);
		            }
			...
			setFilteredModulatorInfoMap(modulatorInfoMap);
		}
...
public static ArrayList<DSMicroarray> createArrayForMindyRun(
     ...
     *
     */
    private void calculateModulatorInfo(boolean recalculate) {
