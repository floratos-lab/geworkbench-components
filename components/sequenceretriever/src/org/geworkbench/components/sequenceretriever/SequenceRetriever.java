...
public class SequenceRetriever implements VisualPlugin {
    ...
    private void getSequences( DSGeneMarker marker ) {
        CSSequence seqs = PromoterSequenceFetcher.getCachedPromoterSequence( marker, ( ( Integer ) model.getNumber() )
            ...
            if ( uniprotids != null ) {
                for ( int i = 0; i < uniprotids.length; i++ ) {
                    if ( uniprotids[i] != null && !uniprotids[i].trim().equals( "" ) ) {
                        String[] result = ( String[] ) call.invoke( new Object[] { "uniprot:" + uniprotids[i], "fasta",
                                "raw" } );
    ...
    @Publish
    public ProjectNodeAddedEvent publishProjectNodeAddedEvent( org.geworkbench.events.ProjectNodeAddedEvent event ) {
        return event;
    }
