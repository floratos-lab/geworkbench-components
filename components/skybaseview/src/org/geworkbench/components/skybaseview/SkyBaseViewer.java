...
public class SkyBaseViewer implements VisualPlugin
{
    ...
    String[] columnNames = {"Model Rank", "Id% Query-Model Sequence", "Model Start-End", "Query Start-End", "Model SeqID", "Model Sequence", "Query Sequence", "pG", "Coverage Template", "Id% Template-Model Sequences", "Template", "Template Length", "eValue", "Model Length", "Model Coverage", "Model Species", "Model Description", "Model File"};
...
//	pdburl = "http://156.145.102.40/pdb/08-13-2008AAK22092.1.pdb";
	...
	else
	{
		...
		String res = result.substring(0, result.lastIndexOf("blout.hits"))+"res";
		br = new BufferedReader(new InputStreamReader(new URL(blastroot+res).openConnection().getInputStream()));
		i = 0;
		...
		while((line = br.readLine()) != null)
		{
		    ...
		    ArrayList<String> al = new ArrayList<String>(Arrays.asList(first[i]));
		    second[i] = new String[]{toks[20], toks[10], toks[5], toks[1], toks[13], toks[4], toks[9], toks[6], toks[14], toks[17], toks[32]};
		    ...
		    System.out.println(mod[i]+": "+pctid[i]+": "+cov[i]+": "+pg[i]);

		    //rank model quality by sorting models in groups with .1 increment of pG:
		    //quality of models with pg in [0.9, 1] > [0.8, 0.9) > [0.7, 0.8) > [0, 0.7)
		    //for each group, rank models by sorting template coverage, then sequence identity%
		    int pggrp = 0;
		    if (pg[i] >= 0.9)
			pggrp = 3;
		    else if (pg[i] >= 0.8)
	    ...
	    sp.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("SkyBase Blast Results for "+qname),BorderFactory.createEmptyBorder(5, 5, 5, 5)),sp.getBorder()));
	...
	public Object getValueAt(int row, int col) {
	    if (col == 0) return Integer.valueOf((String)data[row][col]);
	    return data[row][col];
	}
	...
	public void valueChanged(ListSelectionEvent e){
		...
		{
		    String pdblink = (String)tm.getValueAt(selectedRow, colcnt-1);
		    ...
		    jp2.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder(seqid),BorderFactory.createEmptyBorder(5, 5, 5, 5)),border));
    ...
    private ChartPanel plot_model_quality(String[] mod, Double[] cov, Double[] pg, Double[] pctid, Integer[] rank)
    {
	DefaultCategoryDataset[] sets = create_datasets(mod, cov, pg, pctid, rank);

	JFreeChart ch = ChartFactory.createBarChart(null, null, "pG / Coverage / Identity", sets[0], PlotOrientation.VERTICAL, false, true, false);
	ch.setTitle(new TextTitle("Models for SkyBase Blast Results", new Font("Arial", Font.BOLD, 12)));
	...
	cp.addChartMouseListener(new ChartMouseListener(){
		public void chartMouseClicked(ChartMouseEvent ev){
			...
			{
			    CategoryItemEntity cie = (CategoryItemEntity)ev.getEntity();
    ...
    private DefaultCategoryDataset[] create_datasets(String[] mod, Double[] cov, Double[] pg, Double[] pctid, Integer[] rank)
    {
