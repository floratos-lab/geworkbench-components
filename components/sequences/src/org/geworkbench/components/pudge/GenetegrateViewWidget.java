...
import edu.columbia.pudge.client.PUDGEDEMOClient;
...
public class GenetegrateViewWidget extends JPanel {
    ...
    /**
     * create a sequence per file from the original sequence file and update the query UI.
     */
    public void preProcess() {
        ...
        for (int i = 0; i < activeSequenceDB.size(); i++) {
            ...
            if (fastaFilename.lastIndexOf(".") > 0) {
                name = fastaFilename.substring(0, fastaFilename.lastIndexOf("."));
            }
    ...
    public void runPDBGeneratingFunction() {
        ...
        if (inputFileNames == null || inputFileNames.length == 0) {
            JOptionPane.showMessageDialog(null, "No sequence is activated.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
            ...
            while (!finished) {
                ...
                if (!functionName.equalsIgnoreCase(PUDGE)) {
                   ...
                   DSSequence seq = activeSequenceDB.getSequence(sequenceCount);
                    String jobID = null;
                    try {
                        PUDGEDEMOClient pudgedemoClient = new PUDGEDEMOClient(PUDGEURL);
    ...
    public void updateBottomPanel() {
        ...
        if (goLeft) {
            if (xStartPoint - GenetegrateViewWidget.GAP > 0) {
                xStartPoint = xStartPoint >= GenetegrateViewWidget.GAP ? xStartPoint - GenetegrateViewWidget.GAP : 1;
            } else {
                xStartPoint = 1;
                leftShiftButton.setEnabled(false);
            }
        } else {
            if (xStartPoint < selectedSequence.length() - GenetegrateViewWidget.GAP) {
...
//                                         isLineView);
    ...
    public boolean initPanelView() {
        ...
        isLineView = jViewComboBox.getSelectedItem().equals(GenetegrateViewWidget.LINEVIEW);
...
//                                         isLineView);
    ...
    private class JDetailPanel extends JPanel {
        public void paintComponent(Graphics g) {
            ...
            if (xStartPoint >= 0 &&
                    xStartPoint < selectedSequence.length() - 2 * GenetegrateViewWidget.GAP) {
            ...
            if ((seqId >= 0) && (seqId != prevSeqId) || (seqDx != prevSeqDx)) {
                ...
                if (sequence != null && (seqDx >= 0) &&
                    ...
                    if (seqDx > GenetegrateViewWidget.GAP) {
                        startPoint = seqDx / 10 * 10 - GenetegrateViewWidget.GAP;
                    }
