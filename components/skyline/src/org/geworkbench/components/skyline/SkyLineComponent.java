 ...
 * 
 * @author mw2518
 * @version $Id: SkyLineComponent.java,v 1.10 2009-02-18 22:02:49 chiangy Exp $
 */
...
public class SkyLineComponent extends JPanel implements VisualPlugin {
	...
	class MyRunnable implements Runnable {
		...
		public void run() {
			...
			while (check_log_stat(mylogfile) < 1) {
				if (!change) {
					thread4pdb.put(mylogfile, new String("run"));
				}
				change = true;
				try {
	...
	private void jbInit() throws Exception {
		...
		pluginStructureAnalysis.getSelectionModel().setSelectionMode(
				ListSelectionModel.SINGLE_SELECTION);
		pluginStructureAnalysis
		...
		pluginStructureAnalysis.setBorder(BorderFactory
	 ...
	 * 
	 * @param pe
	 *            Project event cotnaining the newly selected protein structure.
	 */
	// remove redundant thread for job status checking(already done in
	// skylineview component)
	@Subscribe
	...
	public void getAvailableAnalyses() {
		...
		ProteinStructureAnalysis[] analyses = ComponentRegistry.getRegistry()
				.getModules(ProteinStructureAnalysis.class);
		...
		// If the selectedStructureAnalysis has been removed from the list of
		// available
		// normalizers, reset.
		if (selectionChanged)
	...
	/**
	 * Obtains from the PluginRegistry and displays the set of available
	 * analysis
	 */
	public void reset() {
	...
	private void displayStructureAnalysis() {
		...
		if (selectedStructureAnalysis != null)
			pluginStructureAnalysis.setSelectedValue(selectedStructureAnalysis
					.getLabel(), true);
		else {
	...
	private void structureAnalysisSelected_action(ListSelectionEvent lse) {
		if (pluginStructureAnalysis.getSelectedIndex() == -1)
			return;
		selectedStructureAnalysis = availableStructureAnalysis[pluginStructureAnalysis
				.getSelectedIndex()];
		// Set the parameters panel for the selected normalizer.
		ParameterPanel paramPanel = selectedStructureAnalysis
				.getParameterPanel();
		// Set the list of available named parameters for the selected
		// normalizer.
		if (paramPanel != null) {
			setParametersPanel(paramPanel);
			setNamedParameters(availableStructureAnalysis[pluginStructureAnalysis
	...
	private void namedParameterSelection_action(ListSelectionEvent e) {
		...
		if (index != -1)
			selectedStructureAnalysis.setParameters(selectedStructureAnalysis
	 ...
	 * 
	 * @param e
	 *            <code>ActionEvent</code> generated by the "submit" button
	 */
	private void submit_actionPerformed(ActionEvent e) {
		if (handle_status(pname) == false)
			return;

		// job execution stopped with error if log file isn't complete and
		// there's no qjob on the cluster
		if (check_log_stat(logfile) == 0) {
			int response = JOptionPane
					.showConfirmDialog(
							null,
							"SkyLine job exited with error. Are you sure to rerun the analysis?",
		...
		// previous results detected
			...
			int response = JOptionPane
					.showConfirmDialog(
							null,
							"Previous SkyLine results detected. Are you sure to rerun the analysis?",
							"Structure Analysis Status",
							JOptionPane.YES_NO_OPTION);
			if (response == 1)
				return;
		...
		AlgorithmExecutionResults results = selectedStructureAnalysis
				.execute(maSet);
		...
		if (!results.isExecutionSuccessful()) {
			JOptionPane.showMessageDialog(null, results.getMessage(),
					"Structure Analysis Execution Error",
		...
		JOptionPane.showMessageDialog(null, htmlText,
				"Structure Analysis Execution Status",
	...
	/*
	 * listener invoked when "check results" button is clicked
	 */
	private void checkResults_actionPerformed(ActionEvent e) {
		...
		// job hasn't been run before
			...
			JOptionPane.showMessageDialog(null,
					"SkyLine job needs to be submitted",
					"Structure Analysis Status",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	private boolean handle_status(String pname) {
		if (selectedStructureAnalysis == null || maSet == null)
			return false;
		ParamValidationResults pvr = selectedStructureAnalysis
				.validateParameters();
		if (!pvr.isValid()) {
		...
		if (status.equals("not connected")) {
			JOptionPane.showMessageDialog(null,
					...
					"Structure Analysis Status", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		// job is running if the qjob is on the cluster
		if (status.length() > 0) {
			JOptionPane.showMessageDialog(null,
					"<html>SkyLine job hasn't finished<hr>" + status,
					"Structure Analysis Status",
					JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
	...
	private String get_qstat(String pname) {
		...
		if (pname.length() > maxlen_qname)
			pname = pname.substring(0, maxlen_qname);
		try {
			status = ((SkyLineAnalysis) selectedStructureAnalysis)
					.getJobStatus(pname);
		} catch (Exception ce) {
			ce.printStackTrace();
		}
	...
	// @Publish public org.geworkbench.events.NormalizationEvent
	// publishNormalizationEvent(org.geworkbench.events.NormalizationEvent
	// event) {
	@Publish
	public org.geworkbench.events.StructureAnalysisEvent publishStructureAnalysisEvent(
			org.geworkbench.events.StructureAnalysisEvent event) {
		return event;
	}
