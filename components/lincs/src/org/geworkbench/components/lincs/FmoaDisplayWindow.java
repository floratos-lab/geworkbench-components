 ...
 * 
 */
public class FmoaDisplayWindow  {
	...
	private final static String[] fmoaDataColumnNames = { "Gene Name",
		"NES", "Odds Ratio", "Include" };
	...
	public void initComponents() {
		...
		JLabel fMoAGenesWithBlankLabel = new JLabel("fMoA genes                    ");		
        ...
        fmoaDataTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
					...
					{   
						if (((Boolean) target.getModel().getValueAt(row, 3)).booleanValue() == false)
	...
	private Object[][] convertFmoaResultData(List<FmoaResult> dataList)
	{
		...
		for (int i = 0; i < dataList.size(); i++) {
			objects[i][0] = new ValueObject(dataList.get(i).getGeneSymbol(), dataList.get(i).getGeneId());
	...
	private List<ValueObject> getGeneIdList()
	{
		...
		for (int cx = 0; cx < fmoaDataTable.getRowCount(); cx++) {
			if (! TableViewer.isInclude(fmoaDataTable, cx))
				continue;
			geneList.add((ValueObject)fmoaDataTable.getModel().getValueAt(cx, colIndex));
