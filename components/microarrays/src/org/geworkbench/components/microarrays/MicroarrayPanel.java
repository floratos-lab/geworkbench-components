...
public class MicroarrayPanel extends MicroarrayVisualizer implements
	...
	private boolean selectMicroarray(int mArrayId) {
		if (mArraySet != null && dataSetView.items().size() > 0) {
			...
			if (mArray != null) {
				...
				// subscriber.notifyChange(this,
				// IMicroarrayIdChangeSubscriber.class);
				// subscriber.notifyChange(this,
				// IMarkerIdClickSubscriber.class);
				try {
					microarrayImageArea.repaint();
	...
	void microarrayImageArea_mouseMoved(MouseEvent event) {
		if (mArraySet != null) {
			...
			if ((markerId >= 0) && (markerId < mArraySet.size())
				...
				// this is not right, markerID is just the relative position of
				// the marker in current image
				// it can't be used to find the marker in
				// microarryset++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!!!!!
				// subscriber.notifyChange(this,
				// IMarkerIdChangeSubscriber.class);
			}
		}
	}
	...
	void jShowMarkerMenu_actionPerformed(ActionEvent e) {
		...
		// markerPlot.notifyMarkerGraph(microarraySet, microarrayId, markerId,
		// true);
	...
	void jRemoveMarkerMenu_actionPerformed(ActionEvent e) {
		// markerPlot.notifyMarkerGraph(microarraySet, visualizer.microarrayId,
		// visualizer.markerId, false);
	...
	void jSaveImageMenu_actionPerformed(ActionEvent e) {
		...
		if (currentImage != null && microarrayImageArea.microarray != null) {
			...
			// Image scaled = currentImage.getScaledInstance(width, height,
			// Image.SCALE_FAST);
