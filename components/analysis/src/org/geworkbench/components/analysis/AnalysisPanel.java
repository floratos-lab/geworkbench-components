 ...
 * clustering options.
 */
@AcceptTypes( { DSMicroarraySet.class, AdjacencyMatrixDataSet.class, EdgeListDataSet.class })
public class AnalysisPanel extends MicroarrayViewEventBase implements
	...
	private void removeNamedParameter(String name){
		...
		this.setNamedParameters(selectedAnalysis.getNamesOfStoredParameterSets());		
	}
	...
	private void delete_actionPerformed(ActionEvent e){
		...
		if((selectedAnalysis != null) && (choice == 0) && (this.namedParameters.getSelectedIndex() >= 0)){
			log.info("Deleting saved parameters: " + (String) this.namedParameters.getSelectedValue());
			this.removeNamedParameter((String) this.namedParameters.getSelectedValue());
