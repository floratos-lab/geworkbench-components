...
public abstract class AbstractTrainingPanel extends AbstractSaveableParameterPanel implements Serializable, TrainingProgressListener {
                    ...
                    protected Object doInBackground() throws Exception {
                        ...
                        try {
                            for (int i = 0; i < cross.getNumFolds() && !isCancelled(); i++) {
                                ...
                                Classifier classifier = trainForValidation(trainingCaseData, trainingControlData);
                                log.debug("Classifier training complete.");

                                int numPositive = 0;
                                for (float[] values : crossData.getTestCaseData()) {
                                    if (classifier.getClassifications()[0].equals(classifier.classify(values))) {
                                        numPositive++;
                                    }
                                }
    ...
    protected abstract Classifier trainForValidation(java.util.List<float[]> trainingCaseData, java.util.List<float[]> trainingControlData) throws ClassifierException;
