...
import org.geworkbench.components.gpmodule.gsea.browser.TabBrowser;
import org.geworkbench.components.gpmodule.gsea.browser.CloseableTabbedPane;
...
public class GSEABrowser implements VisualPlugin
{
    ...
    private void display()
    {
        ...
        {
            if (is_mac)
		        setURL.invoke(webBrowser, gseaURL);
		    else if (!is_windows)
			{
				...
				jtp.addTab(gseaResultDataSet.getReportFile(), jp);
				mainPanel.add(jtp, BorderLayout.CENTER);
				mainPanel.invalidate();
				mainPanel.repaint();
		    }
    ...
    {        
        ArchiveUtil.unzipRecursive(new File(gseaResultDataSet.getReportFile()), new File(System.getProperty("temporary.files.directory")));

        System.out.println("report file:" + gseaResultDataSet.getReportFile());
        try
        {
            gseaURL = (new File(System.getProperty("temporary.files.directory") +"index.html")).toURL().toString();
        }
