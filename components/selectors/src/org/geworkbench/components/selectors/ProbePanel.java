 ...
 */
public class ProbePanel extends SelectorPanel<CSPSAMMatch> {
    ...
    public ProbePanel() {
        super(CSPSAMMatch.class, "Marker");
    ...
    private void loadButtonPressed() {
        ...
        if (choice == JFileChooser.APPROVE_OPTION) {
            DSPanel<CSPSAMMatch> panel = deserializePanel(fc.getSelectedFile());
            addPanel(panel);
            throwLabelEvent();
        }
    }
...
//        }
    ...
    private void serializePanel(String filename, String[] labels) {
        ...
        try {
            ...
            for(int i = 0; i < labels.length; i++){
                DSPanel<CSPSAMMatch> panel = context.getItemsWithLabel(labels[i]);
                if (panel != null && panel.size() > 0) {
                    for (int j = 0; j < panel.size(); j++) {
                        CSPSAMMatch marker = panel.get(j);
     ...
     */
    private DSPanel<CSPSAMMatch> deserializePanel(final File file) {
        ...
        DSPanel<CSPSAMMatch> panel = new CSPanel<CSPSAMMatch>(filename);
        try {
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                if (line.length > 0) {
                    String label = line[0];
                    CSPSAMMatch marker = itemList.get(label);
                    if (marker != null) {
                        panel.add(marker);
                    }
                }
    ...
    protected void publishSingleSelectionEvent(CSPSAMMatch item) {
//        publishGeneSelectorEvent(new GeneSelectorEvent(item));
    ...
    public DSPanel createPanels(String label, int[] positions) {
        ...
        DSPanel<CSPSAMMatch> panel = new CSPanel<CSPSAMMatch>(label);
        for (int position : positions) {
            CSPSAMMatch marker = itemList.get(position);
            if (marker != null) {
                panel.add(marker);
            }
        }
...
//        publishGeneSelectorEvent(new GeneSelectorEvent(panel));
    ...
    public DSPanel getPanel(String dir) {
        DSPanel<CSPSAMMatch> panel = deserializePanel(new File(dir));
        panel.setActive(true);
        addPanel(panel); //redundancy between broadcast and script
//        publishGeneSelectorEvent(new GeneSelectorEvent(panel));
