...
/**
 * @author keshav
 * @version $Id: GridUtils.java,v 1.1 2007-01-10 17:14:28 keshav Exp $
 */
public class GridUtils {
    ...
    public static float[][] readTabFile( InputStream is ) {
        ...
        while ( iter.hasNext() ) {
            ...
            for ( int j = 0; j < values[i].length; j++ ) {
                 ...
                 * actual values. Inspects the data, row by row. If it starts with a character, then this is the header,
                 * if it ends with _at, then this is a probe name. FIXME I also don't like the _at check here as not all
                 * probes will end with this. You need something more generic.
                 */
                if ( array[j].equalsIgnoreCase( "probe" ) ) break;
                if ( Character.isLetter( array[j].charAt( 0 ) ) || array[j].endsWith( "_at" ) ) continue;
    ...
    public static Object getHeader() {
        ...
        if ( parser == null ) throw new RuntimeException( "Null header.  Try calling readTabFile first." );
